...
{
     PS_Fill_Circle(PS_Current_File,cx0,cy0,R,filling);
}
...
void PS_Fill_Rectangle(double x0, double y0, double wx, double wy, 
		       double filling)
{
     PS_Fill_Rectangle(PS_Current_File,x0,y0,wx,wy,filling);
}
...
void PS_Fill_Polygon(FILE *psfile, 
		    const Vector &X, const Vector &Y, double filling)
{
     fprintf(psfile,"n\n");
     fprintf(psfile,"%.4g %.4g m\n",X(1),Y(1));
     for (long i=2;i<=X.N;i++)
...
{
     PS_Fill_Polygon(PS_Current_File,X,Y,filling);
}
